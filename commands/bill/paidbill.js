...
module.exports = {
  ...
  usage: "/paidbill <all | mã | ngày/tháng/năm | tháng/năm>",

  async paidBills(ctx, bills) {
    if (bills.length === 0) {
      ctx.reply(
  ...
  async execute(ctx, args) {
    if (args.length === 0) {
      return ctx.reply(
        `❌ *Thiếu mã hóa đơn!*\n\n` +
          `*Cách dùng:* /paidbill <all |mã | ngày/tháng/năm | tháng/năm>\n\n` +
    ...
    try {
      if (firstArgs === "all") {
        const bills = await Bill.find({ userId: ctx.from.id, isPaid: false });

        const result = await this.paidBills(ctx, bills);
        if (!result) {
          return;
        }

        return ctx.reply(
          `✅ *Đã đánh dấu thanh toán cho tất cả hóa đơn của bạn!*`
        );
        ...
        return ctx.reply(
          `✅ *Đã đánh dấu thanh toán cho hóa đơn của bạn trong ngày ${date.toLocaleDateString(
            "vi-VN"
          )}!*`
        );
        ...
        return ctx.reply(
          `✅ *Đã đánh dấu thanh toán cho hóa đơn của bạn trong tháng ${
